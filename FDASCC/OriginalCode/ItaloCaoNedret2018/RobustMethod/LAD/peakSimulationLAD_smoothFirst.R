...
peakSimLADSmoothFirst <- function(contamination, n, p = 4, Nm = 0, c = 0.5, sigma = 0.5, alpha = 0.05, strength = 10, correction = 0, debug = FALSE) { 
  #Constants ----
  ...
  #Number of  knots
  ## Nm == 0 indicates that no Nm is pre-established
  ##    and resorts to the basic formula bellow
  if(Nm == 0) {
    Nm <- floor(c*(n^(1/(2*p+1))*log(n)))
  }
  
  #Definition of Correction Factor
  ## Current correction factor 1 + a / (1 + n^b),
  ##   where a and b are parameters to be estimated.
  ##   
  if(correction == 0) {
    # correction = 1.3*1.1^(log(n/30, base = 2))
    # correction = 3/(1+n^0.25)
    
    correction = 1 + 2/(1 + n^0.1)
  }
  #####
  
  # Number of measurements points for testing the cofidence band
  N.test = 100
  ...
  for (a in 1 : maxSim){
    sample <- rbind(generateSample1(n.real, N, sigma), generateOutlierPeak(numberOfSamples =  n.out, N, strength = strength, sigma))
