...
structure.true=c("AR1") #"IND", "EX", "AR1",
...
mpr=0.9 #seq(1,0.8,by=-0.2/J)       ## 1-mpr: missing probability
...
for(kkk in 1:length(n_num)){	
  ...
  #############################################  
    ...
    for(kk in 1:nsims){
      ...
      Q.est=generateQNon(J,covM,phiavg.data,lamdaK.data,GMean,alpha,indexka)
      Q=Q.est$Q
